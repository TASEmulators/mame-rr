	...
	},
	{
		...
		box_address = function(obj, box, box_entry)
			...
			if memory.readword(obj.base + 0x1A) == 0x2C and box.type == "proj. attack" then --lasers
				...
				obj.dx        = memory.readwordsigned(address + 0x2) * (obj.facing_dir > 0 and -1 or 1)
...
for game in ipairs(profile) do
	...
	g.offset.y_position = g.offset.x_position + 0x4
...
local function update_object(obj)
	...
	obj.pos_y      = memory.readwordsigned(obj.base + game.offset.y_position) + obj.pos_z
...
local function update_hitboxes()
	...
	if not game.address.left_screen_edge[camera_mode] then
		camera_mode = 1
	end
	...
	frame_buffer[DRAW_DELAY+1] = {game_phase = memory.readbyte(game.address.game_phase)}
	for _,set in ipairs(game.objects) do
		for n = 1, set.number do
			...
			if memory.readwordsigned(obj.base) >= game.exist_val then
				...
				update_object(obj)
				table.insert(frame_buffer[DRAW_DELAY+1], obj)
				if obj.box_count then
					frame_buffer[DRAW_DELAY+1].box_count = obj.box_count
				end
			end
		end
	end
...
local function render_hitboxes()
	...
	if not game or frame_buffer[1].game_phase == 0 then
		return
	end
	...
	for entry = 1, frame_buffer[1].box_count or #game.box_list do
