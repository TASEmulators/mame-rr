...
void WinLuaOnStop(int hDlgAsInt)
{
	...
	if(prevWindow == win_window_list->hwnd) SetActiveWindow(prevWindow);
...
INT_PTR CALLBACK DlgLuaScriptDialog(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		GetWindowRect(win_window_list->hwnd, &r);
		...
		SetDlgItemTextA(hDlg, IDC_EDIT_LUAPATH, MAME_GetLuaScriptName());
	...
	case WM_COMMAND:
		switch (LOWORD(wParam)) {
			...
			case IDC_BUTTON_LUAEDIT:
			{
				...
				if((int)ShellExecute(NULL, L"edit", Str_Tmp, NULL, NULL, SW_SHOWNORMAL) == SE_ERR_NOASSOC)
					if((int)ShellExecute(NULL, L"open", Str_Tmp, NULL, NULL, SW_SHOWNORMAL) == SE_ERR_NOASSOC)
						ShellExecute(NULL, NULL, L"notepad", Str_Tmp, NULL, SW_SHOWNORMAL);
			}	break;
...
//				AudBlankSound();
				...
				luaofn.lpstrFilter = L"Lua scripts (*.lua)\0*.lua\0All files (*.*)\0*.*\0\0";
				luaofn.lpstrFile = szFileName;
				luaofn.lpstrDefExt = L"lua";
