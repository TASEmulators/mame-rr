...
struct InitRamWatch
{
	InitRamWatch()
	{
		GetModuleFileNameA(NULL, applicationPath, 2048);
	}
} initRamWatch;
			...
			switch(LOWORD(wParam))
			{
				...
				case IDOK:
				{
					if (s && t)
					{
						...
						if((Temp.Address & ~0xFFFFFFFF) == ~0xFFFFFFFF)
...
//							Item->item.mask = LVIF_TEXT; // crashes
							...
							switch (Item->item.iSubItem)
							{
								...
								case 2:
									Item->item.pszText = _AtoT(rswatches[iNum].comment ? rswatches[iNum].comment : (char*)"");
									return true;
